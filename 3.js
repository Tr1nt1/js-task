...
for (let i = 0; i < numbers.length; i++ ) {
    console.log('Индексу ' + i + ' соответстыует число ' + numbers[i]);
}
...
for (let i = 0; i < vegetables.length; i++) {
    characteristic.push({word: vegetables[i], length: vegetables[i].length})
}
for (let item of characteristic) {
